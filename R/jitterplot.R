...
#'
#' @description
#' Creates a single jitter plot without splitting. Supports point sizing, labeling,
#' highlighting, and optional background layers.
#'
#' @inheritParams parameters
#' @keywords internal
#' @importFrom stats median quantile reorder
#' @importFrom rlang sym syms parse_expr %||%
#' @importFrom dplyr mutate ungroup group_by arrange row_number
#' @importFrom ggplot2 aes geom_point labs theme element_line element_text coord_flip
#' @importFrom ggplot2 position_jitterdodge scale_fill_manual scale_color_manual
#' @importFrom ggplot2 scale_y_continuous scale_size_area guide_legend guides scale_x_discrete
#' @importFrom ggplot2 geom_hline coord_cartesian
...
JitterPlotAtomic <- function(
    ...
    labels = NULL, label_by = NULL, nlabel = 5, label_size = 3, label_fg = "black",
    label_bg = "white", label_bg_r = 0.1,
  ...
  # Handle wide/long format
  in_form <- match.arg(in_form)
  if (in_form == "wide") {
    data <- tidyr::pivot_longer(data, cols = x, names_to = ".x", values_to = ".y")
    x <- ".x"
    y <- ".y"
  }
  ...
  # Calculate x-axis sorting metrics
  sort_x <- match.arg(sort_x)
  data <- data |>
    dplyr::group_by(!!!syms(unique(c(x, group_by, facet_by)))) |>
    dplyr::mutate(.y_mean = mean(!!sym(y)), .y_median = stats::median(!!sym(y))) |>
    dplyr::ungroup()
  ...
  if (isTRUE(add_bg)) {
    p <- p + create_bg_layer(
      ...
      direction = if (flip) "horizontal" else "vertical"
    )
  }

  # Positioner (jitter + optional dodge)
  pos <- ggplot2::position_jitterdodge(
  ...
  if (has_fill) {
    mapping[[length(mapping) + 1]] <- ggplot2::aes(fill = !!sym(x))
    # Border handling
    if (isTRUE(border)) {
      mapping[[length(mapping) + 1]] <- ggplot2::aes(color = !!sym(x))
  ...
  if (is.numeric(size_by)) {
    ...
    mapping[[length(mapping) + 1]] <- ggplot2::aes(size = !!sym(".size_mapped"))
  }

  # Group for dodging only (no legend)
  if (!is.null(group_by)) {
    mapping[[length(mapping) + 1]] <- ggplot2::aes(group = !!sym(group_by))
  }
  ...
  if (has_fill) {
    p <- p + ggplot2::scale_fill_manual(
      name = x, values = get_palette(x_levels, palette = palette, palcolor = palcolor),
      drop = !keep_empty
    )
    if (isTRUE(border)) {
      p <- p + ggplot2::scale_color_manual(
        values = get_palette(x_levels, palette = palette, palcolor = palcolor),
        guide = "none", drop = !keep_empty
      )
    }
  } else {
    p <- p + ggplot2::scale_color_manual(
  ...
  if (!is.numeric(size_by)) {
    ...
    p <- p + ggplot2::scale_size_area(max_size = 6, breaks = mapped_breaks, labels = raw_breaks) +
    ...
    if (has_fill) {
      p <- p + ggplot2::geom_point(
        ...
        mapping = if (!is.null(group_by)) {
          ggplot2::aes(x = !!sym(x), y = !!sym(y), group = !!sym(group_by))
        } else {
          ggplot2::aes(x = !!sym(x), y = !!sym(y))
        },
        shape = shape, fill = highlight_color, color = "transparent",
        position = pos, size = if (is.numeric(size_by)) highlight_size else highlight_size,
  ...
  if (any(data$.show_label)) {
    p <- p + ggrepel::geom_text_repel(
      ...
      mapping = ggplot2::aes(label = !!sym(".label")),
  ...
  if (isTRUE(flip)) {
    ...
    p <- p + ggplot2::theme(
      ...
      panel.grid.major.y = ggplot2::element_line(color = "grey", linetype = 2)
    )
    if (facet_free) {
    ...
    p <- p + ggplot2::theme(
      ...
      panel.grid.major.x = ggplot2::element_line(color = "grey", linetype = 2)
    )
    if (!facet_free) {
...
#' Supports point sizing, dodge grouping, and various customization options.
#'
#' @inheritParams parameters
