...
#' @param cutoffs_accuracy A numeric to specify the accuracy of the cutoff values.
...
get_cutoffs_data <- function(
  ...
  if (is.null(cutoffs_at) && n_cuts > 0) {
    cutoffs_at <- stats::quantile(data[[score_by]], probs = seq(0, 1, length.out = n_cuts + 2))
    cutoffs_at <- unname(cutoffs_at[-c(1, length(cutoffs_at))])
  }

  stopifnot(
    "cutoffs_labels should be NULL or a character vector of the same length as cutoffs_at." =
...
#' @importFrom ggplot2 geom_text labs geom_point element_line
#' @importFrom rlang sym syms "%||%"
ROCCurveAtomic <- function(
    data, truth_by, score_by, pos_label = NULL,
    ...
    theme = "theme_ggforge", theme_args = list(), palette = "Spectral", palcolor = NULL, alpha = 1,
  ...
  truth_by <- validate_columns(data, truth_by, allow_multi = FALSE)
  score_by <- validate_columns(data, score_by, force_factor = FALSE, allow_multi = TRUE)
  ...
  if (length(score_by) > 1) {
    data <- tidyr::pivot_longer(data, cols = score_by, names_to = ".group", values_to = ".score")
    group_by <- ".group"
  ...
  # Get base_size for text scaling
  base_size <- theme_args$base_size %||% 12
  text_size_scale <- base_size / 12

  # Handle NULL group_by by creating a dummy group
  if (is.null(group_by)) {
    data$..group <- factor("")
    group_by <- "..group"
  }
  ...
  legend.position <- normalize_legend_position(
    legend.position,
    has_group = !identical(group_by, "..group")
  )
  ...
  # Filter out parameters that shouldn't be passed to geom_roc
  extra_args <- list(...)
  geom_roc_args <- extra_args[!names(extra_args) %in% c("split_by", "split_by_sep", "combine", "nrow", "ncol", "byrow", "seed", "axes", "axis_titles", "guides", "design")]

  p <- ggplot(data, ggplot2::aes(d = !!sym(truth_by), m = !!sym(score_by), color = !!sym(group_by))) +
    # plotROC::geom_roc() doesn't support different cutoffs for different groups/facets
    do.call(plotROC::geom_roc, c(list(n.cuts = 0, cutoffs.at = NULL, cutoff.labels = NULL, increasing = increasing), geom_roc_args))

  # facet the plot to calculate the correct AUCs for each facet
  attr(p, "height") <- attr(p, "width") <- 0
  p_faceted <- p
  if (!is.null(facet_by)) {
    p_faceted <- add_facets(p_faceted, facet_by, facet_scales, facet_nrow, facet_ncol, facet_byrow)
  }
  auc <- plotROC::calc_auc(p_faceted)

  cutoffs_df <- get_cutoffs_data(
    data %>% tidyr::unite(".cat", !!!syms(unique(c(group_by, facet_by))), sep = " // "),
  ...
  if (!is.null(cutoffs_df)) {
    cutoffs_df <- cutoffs_df %>%
      tidyr::separate(".cat", into = unique(c(group_by, facet_by)), sep = " // ")

    p <- p + ggplot2::geom_point(
      data = cutoffs_df, ggplot2::aes(x = !!sym("x"), y = !!sym("y"), color = !!sym(group_by)), inherit.aes = FALSE, shape = cutoffs_pt_shape,
      size = cutoffs_pt_size, stroke = cutoffs_pt_stroke
    ) + ggrepel::geom_text_repel(
      data = cutoffs_df, ggplot2::aes(label = !!sym("label"), x = !!sym("x"), y = !!sym("y")), inherit.aes = FALSE,
  ...
  if (show_auc == "plot") {
    ...
    p <- p + ggplot2::geom_text(
      ...
      hjust = auc_pos_hjust, vjust = auc_pos_vjust, ggplot2::aes(label = !!sym("AUC")), inherit.aes = FALSE
    )
    ...
    if (!is.null(facet_by)) {
      ...
      labels <- auc_df %>%
        ...
        dplyr::summarise(AUC = paste(!!sym("AUC"), collapse = "\n"), .groups = "drop") %>%
        dplyr::mutate(AUC = paste0(!!sym(group_by), "\n", !!sym("AUC"))) %>%
        dplyr::pull("AUC")
    } else {
  ...
  p <- p +
    # diagnal line
    ggplot2::geom_abline(intercept = 0, slope = ifelse(x_axis_reverse, -1, 1), linetype = "dashed", color = "grey50") +
    ggplot2::scale_color_manual(
      name = group_name %||% group_by,
      values = get_palette(levels(data[[group_by]]), palette = palette, palcolor = palcolor, alpha = alpha),
      labels = labels
    )
  ...
  if (x_axis_reverse) {
    p <- p + ggplot2::scale_x_reverse(labels = if (isTRUE(percent)) scales::percent else ggplot2::waiver())
  ...
  # Both axes are continuous probabilities/rates (0-1 or 0-100)
  # Use continuous axis text style
  p <- p + ggplot2::theme(
    ...
    axis.text.y = build_element_text("font.axis_text.continuous", base_size)
  )
...
#' @return A `patchwork::wrap_plots` object or a list of them if `combine` is `FALSE`.
...
#' attr(p, "cutoffs")
ROCCurve <- function(
    data, truth_by, score_by, pos_label = NULL, split_by = NULL, split_by_sep = "_",
  ...
  # If we have split_by, we need to handle attributes specially
  if (!is.null(split_by) && combine) {
    # Get the plots first without combining
    plot_list <- build_plot(
      data = data,
      atomic_fn = ROCCurveAtomic,
      params = params,
      split_by = split_by,
      facet_by = facet_by,
      combine = FALSE
    ...
    # Extract and aggregate attributes
    split_by_col <- validate_columns(data, split_by, force_factor = TRUE, allow_multi = TRUE, concat_multi = TRUE, concat_sep = split_by_sep)
    split_levels <- names(plot_list)

    # Aggregate AUC
    auc_list <- lapply(seq_along(plot_list), function(i) {
      auc_df <- attr(plot_list[[i]], "auc")
      if (!is.null(auc_df)) {
        auc_df[[split_by_col]] <- split_levels[i]
      }
      auc_df
    })
    ...
    # Aggregate cutoffs
    cutoff_list <- lapply(seq_along(plot_list), function(i) {
      cutoff_df <- attr(plot_list[[i]], "cutoffs")
      if (!is.null(cutoff_df)) {
        cutoff_df[[split_by_col]] <- split_levels[i]
      }
      cutoff_df
    })
    cutoff_list <- cutoff_list[!sapply(cutoff_list, is.null)]

    # Now combine the plots
    result <- combine_plots(
      plot_list,
      combine = TRUE,
      nrow = nrow,
      ncol = ncol,
    ...
    # Attach aggregated attributes
    if (length(auc_list) > 0) {
      attr(result, "auc") <- do.call(rbind, auc_list)
    }
    if (length(cutoff_list) > 0) {
      attr(result, "cutoffs") <- do.call(rbind, cutoff_list)
    }

    return(result)
  } else {
    # No split_by or combine=FALSE, use standard workflow
    result <- build_plot(
      data = data,
      atomic_fn = ROCCurveAtomic,
      params = params,
